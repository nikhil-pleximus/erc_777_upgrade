from brownie import network, Contract, MyTokenV1, web3, ProxyAdmin, TransparentUpgradeableProxy, config
...
def main():
    ...
    my_token = MyTokenV1.deploy({"from": account}, publish_source=config["networks"][network.show_active()].get("verify")) # 0x73Afd6AA2F20d1d3aCA6aEdEa43233dC1A56366A
    ...
    proxy_contract = Contract.from_abi("My Token", proxy.address, MyTokenV1.abi)
